...
export default function Slider<T>({
  ...
  function slide(left: boolean = false) {
    ...
    animate(
      scope.current,
      {
        x: slides.current[index.current] + "px",
      },
      {
        type: "tween",
        duration: 0.3,
  ...
  if (max) {
    for (let i = 0; i < max; i++) {
      const dot = (
        <li key={i} className={`dot ${i == 0 ? "current" : ""}`}></li>
      );
      dots.push(dot);
    }
  }
