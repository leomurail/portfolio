...
export default function StrateSlider({ step, children }: Props) {
  ...
  function handleArrowClick(
    ...
    const sliderContent = clickedElement
      .closest(".card-slider")
    ...
    if (window.innerWidth <= 430) {
      const numberOfCard =
        sliderContent?.getElementsByClassName("project-card").length;
      if (numberOfCard) {
        sliderContentWidth.phone =
          sliderContentWidth.default -
          (sliderContentWidth.default % numberOfCard);
        console.log(
          sliderContentWidth.default % numberOfCard,
          sliderContentWidth.default,
        );
    ...
    slideLength.current = right
      ? slideLength.current + finalWidth
      : slideLength.current - finalWidth;
  ...
  return (
    <div className="card-slider">
      <RoundBtn
        icon="arrow"
        size="m"
        onClick={(event) => handleArrowClick(event, false)}
      />
