  ...
  useFrame(() => {
    ...
    if (totem.current && (xValue || xValue == 0)) {
      if (states.click) {
        ...
        anim.current.xPose +=
          anim.current.xPose > (-15 - xValue) / 2 ? (-15 - xValue) / 2 / 20 : 0;
        ...
        anim.current.xPose -=
          anim.current.xPose < 0 ? (-15 - xValue) / 2 / 20 : 0;
      }

      totem.current.scale.set(
        anim.current.scale,
        anim.current.scale,
        anim.current.scale,
      );
  ...
  return (
    <>
      ...
      >
        ...
        <mesh
          scale={[0.3, 0.3, 0.3]}
          position={[value.position ? value.position.x : 0, 8, 0]}
        >
          <sphereGeometry />
          <meshStandardMaterial
            emissive={"#FFFFFF"}
            emissiveIntensity={5}
            visible={oneIsShown ? !visibility : true}
            opacity={0}
            transparent={true}
            ref={light}
          />
